      ...
      <SheetContent className="sm:max-w-2xl w-full p-0 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-24">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 items-center gap-2 sm:gap-4">
                <Label htmlFor="department" className="text-left sm:text-right">Department</Label>
                <Select value={department} onValueChange={setDepartment}>
                  <SelectTrigger className="col-span-1 sm:col-span-3"><SelectValue placeholder="Select Department" /></SelectTrigger>
                  <SelectContent>{departmentOptions.length > 0 ? departmentOptions.map((d) => <SelectItem key={d} value={d}>{d}</SelectItem>) : <SelectItem value="none" disabled>No Departments in Settings</SelectItem>}</SelectContent>
                </Select>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 rounded-lg border p-4 bg-muted/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-[1fr_100px] gap-2">
                  <div className="grid gap-1.5"><Label className="text-xs text-muted-foreground">Official Paid</Label><Input type="number" placeholder="0.00" value={officialSettlement} onChange={e => setOfficialSettlement(e.target.value)} className="font-mono" /></div>
